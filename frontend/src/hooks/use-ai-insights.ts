...
export function useGenerateInsight() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      // The insight is generated by a Celery task (Claude call) that can take
      // several seconds — poll a few times so it appears without a manual refresh.
      [3000, 6000, 10000, 16000, 24000].forEach((ms) =>
        setTimeout(() => qc.invalidateQueries({ queryKey: ["ai-insights"] }), ms)
      );
    },
  });
}
